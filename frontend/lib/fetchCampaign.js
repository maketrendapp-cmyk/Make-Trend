// lib/fetchCampaign.js
import { refreshDeviceId } from '../utils/deviceId';

const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL;
if (!BACKEND_URL) throw new Error('Missing NEXT_PUBLIC_BACKEND_URL');

const campaignCache = new Map();
const CACHE_TTL = 60 * 1000;

export async function fetchCampaign(id) {
  if (!id) return null;

  // ── Add device ID header only in the browser ──
  const headers = {};
  if (typeof window !== 'undefined') {
    try {
      const deviceId = await refreshDeviceId();
      if (deviceId) headers['x-device-id'] = deviceId;
    } catch (e) { /* ignore */ }
  }

  // ── Cache (same id – header doesn't affect data) ──
  const cached = campaignCache.get(id);
  if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
    return cached.data;
  }

  try {
    const res = await fetch(`${BACKEND_URL}/api/campaigns/${id}`, { headers });
    const data = await res.json();
    const result = data.success ? data.campaign : null;
    if (result) {
      campaignCache.set(id, { data: result, timestamp: Date.now() });
    }
    return result;
  } catch (error) {
    console.error('Error fetching campaign:', error);
    return null;
  }
}

export function invalidateCampaignCache(id) {
  if (id) campaignCache.delete(id);
  else campaignCache.clear();
}