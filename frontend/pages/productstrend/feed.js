...
const ProductCard = React.forwardRef(({
  ...
  let cardClasses = 'bg-white rounded-2xl p-4 hover:shadow-lg transition-all duration-300 flex items-center gap-4 border border-slate-100';
  let badge = null;

  if (isFeatured && rank) {
    // All featured cards use a subtle purple-indigo gradient
    cardClasses += ' bg-gradient-to-br from-purple-50 to-indigo-50 shadow-md border-purple-200/60';
    
    if (rank === 1) {
      badge = (
        <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold bg-yellow-100 text-yellow-800 border border-yellow-200 shadow-sm">
          🥇 #1
        </span>
      );
    } else if (rank === 2) {
      badge = (
        <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold bg-slate-200 text-slate-700 border border-slate-300 shadow-sm">
          🥈 #2
        </span>
      );
    } else if (rank === 3) {
      badge = (
        <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold bg-orange-100 text-orange-800 border border-orange-200 shadow-sm">
          🥉 #3
        </span>
      );
    } else {
      badge = (
        <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold text-purple-600 bg-purple-100 border border-purple-200 shadow-sm">
          🔥 #{rank}
        </span>
      );
    }
          ...
          <div className="flex-1 min-w-0">
            ...
            {isFeatured && badge && (
              <div className="flex items-center gap-2 mt-1">
                {badge}
                {!compact && <span className="text-xs text-purple-400 font-medium">Featured</span>}
              </div>
            )}
          </div>
          <button
...
// ── Featured Carousel (top 3 only) with touch support ──
const FeaturedCarousel = ({ products, onUpvote, isAuthenticated, isUpvoting }) => {
  ...
  return (
    <div className="relative bg-gradient-to-r from-purple-100/60 via-indigo-50/80 to-purple-100/60 rounded-3xl p-6 md:p-8 border border-purple-100/60 shadow-lg mb-10 overflow-hidden">
      ...
      <div className="relative z-10">
        {/* Header – now controls are inside the flex row, not overlapping */}
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm text-slate-500 hidden sm:block">
                Top 3 most upvoted {total > 1 && `– #${currentIndex+1}`}
              </p>
            </div>
          </div>
          {total > 1 && (
