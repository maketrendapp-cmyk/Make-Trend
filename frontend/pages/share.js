...
export default function CampaignShare() {
  ...
  // ===== SHARE ACTION – calls backend, backend decides if to increment =====
  const handleShare = async (platform) => {
    ...
    try {
      ...
      // Refresh share count after share
  ...
  const handleCopyLink = async () => {
    ...
    try {
      ...
      // Call share endpoint – backend will decide if to increment
  ...
  // ===== COMPLETE CAMPAIGN – backend checks conditions =====
    ...
    try {
      ...
      if (data.success) {
        if (campaign?.finalUrl) {
          ...
          alert('🎉 Campaign unlocked successfully!');
          router.push('/');
        }
      } else {
      ...
      <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-border p-8 text-center">
            ...
            {shareCount > 0 && (
              <div className="mt-6 flex flex-wrap gap-3 justify-center">
                ...
                <button
                  ...
                  disabled={isSharing}
                  className="inline-flex items-center gap-2 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition disabled:opacity-50"
                >
                  Telegram
                </button>
                <button
                  ...
                  disabled={isSharing}
                  className="inline-flex items-center gap-2 px-4 py-2 bg-blue-700 text-white rounded-lg hover:bg-blue-800 transition disabled:opacity-50"
                >
                  Facebook
                </button>
                <button
                  ...
                  disabled={isSharing}
                  className="inline-flex items-center gap-2 px-4 py-2 bg-sky-500 text-white rounded-lg hover:bg-sky-600 transition disabled:opacity-50"
                >
                  Twitter
                </button>
              </div>
            ...
            >
              {isCompleting ? 'Processing...' : '🎉 Complete Campaign'}
            </button>
