...
export default function Navbar() {
  ...
  return (
    <>
          ...
          <div className="flex h-12 sm:h-14 items-center justify-between">
            ...
            <Link href="/" className="flex items-center gap-1.5 group flex-shrink-0">
              <img 
                src="/favicon.ico" 
                alt="Make Trend" 
                className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg shadow-md group-hover:shadow-lg transition-all group-hover:scale-105"
              />
              <span className="text-sm sm:text-lg font-extrabold tracking-tight whitespace-nowrap">
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              {isAuthenticated ? (
                <>
                  ...
                  <Link
                    ...
                    "
                  >
                    <div className="w-6 h-6 rounded-full bg-gradient-to-br from-purple-500 to-indigo-500 flex items-center justify-center text-white text-[9px] font-bold shadow-sm flex-shrink-0">
                      {user?.fullname?.charAt(0)?.toUpperCase() || user?.email?.charAt(0)?.toUpperCase() || 'U'}
                    </div>
                    <span className="text-xs font-semibold text-gray-700 group-hover:text-gray-900 transition truncate max-w-[60px] sm:max-w-[100px]">
              ...
              <button
                ...
                className="md:hidden p-1.5 rounded-lg text-gray-600 hover:bg-gray-100 hover:text-gray-900 transition-all duration-200 flex-shrink-0"
      ...
      <div
        className={`
          md:hidden fixed inset-x-0 top-12 sm:top-14 z-40 
          bg-white/95 backdrop-blur-lg border-b border-gray-200
          shadow-xl transition-all duration-300 ease-in-out
          ${isMobileMenuOpen 
            ? 'max-h-[calc(100vh-3rem)] opacity-100 translate-y-0' 
            : 'max-h-0 opacity-0 -translate-y-4 overflow-hidden'
          }
        `}
      >
        <div className="p-3 space-y-0.5 overflow-y-auto max-h-[calc(100vh-3rem)]">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
          ...
          <div className="pt-2 border-t border-gray-100 mt-2">
            {isAuthenticated ? (
              ...
              <Link
                href="/auth/login"
                onClick={() => setIsMobileMenuOpen(false)}
                className="flex items-center justify-center px-4 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-semibold rounded-lg hover:from-purple-700 hover:to-indigo-700 transition-all duration-200 w-full"
              >
                Get Started
              </Link>
            )}
