...
export default function Sidebar() {
  ...
  return (
    <div className="flex flex-col h-full bg-white/95 backdrop-blur-sm border-r border-gray-200">
      {/* ── Navigation Links (tight spacing) ── */}
      <nav className="flex-1 overflow-y-auto px-2 py-1 space-y-0.5">
        {navItems.map((item) => (
          <Link
            ...
            href={item.href}
            className={`
              flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-sm font-medium transition-all duration-200
              ${isActive(item.href)
          ...
          >
            ...
            {isActive(item.href) && (
              <span className="ml-auto w-1 h-4 rounded-full bg-purple-600" />
            )}
          </Link>
        ))}
      </nav>

      {/* ── User Info + Logout (at bottom, compact) ── */}
      <div className="flex-shrink-0 border-t border-gray-200 p-2 bg-gray-50/50">
        {isAuthenticated ? (
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-white font-bold text-xs shadow-md overflow-hidden flex-shrink-0">
                {profileLoading ? (
              ...
              {isPro && (
                <span className="flex items-center gap-0.5 text-[8px] font-bold text-yellow-600 bg-yellow-100 px-1.5 py-0.5 rounded-full flex-shrink-0">
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-lg px-2 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50 transition-all duration-200 border border-red-100 hover:border-red-200"
            >
          ...
          <Link
            href="/login"
            className="flex w-full items-center justify-center gap-2 rounded-lg px-2 py-1.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-semibold text-xs hover:shadow-lg transition-all duration-200"
          >
            <FiUser className="w-3.5 h-3.5" />
            Sign In
          </Link>
        )}
        <p className="text-[8px] text-gray-400 text-center mt-1">v2.0.0</p>
      </div>
    </div>
  );
}